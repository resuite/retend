...
`input` and `textarea` share default control styling: comfortable padding, a 6px corner radius, a white surface, and a hairline border. Author declarations override the defaults per declared field, so a form field needs no surface styling of its own. Both accept `placeholder`, which the native editor renders while the control is empty.

### Image handling

`img` delegates loading, decoding, caching, intrinsic image metadata, and rendering to GPUI's image system. The native bridge accepts HTTP(S) and `file:` `src` URLs. `http(s)` sources load through GPUI's asset source; `file:` sources load from the filesystem, which is how bundled assets reach the native side. `VITE.md` defines the production asset pipeline: the build emits imported assets as real files in the bundle's resource directory and the runtime rewrites root-relative imports (`/assets/...`) into `file:` URLs. Bare relative paths remain outside the native surface and fail soft. Rust stores the accepted URL and parsed `objectFit` value as Retend-native image state. `null` removes the source; a non-string/non-null wire value is a renderer contract failure, while a string that is not a valid supported URL fails soft as an invalid application value and leaves the source absent. A source update is reflected on the next fresh GPUI render. When `src` is absent or cleared, Retend still renders the same stable-ID GPUI `Img` with an empty custom source, preserving its styled layout participation and GPUI element identity without initiating a resource load. GPUI owns loading state, animation state, and shared resource caching, so Retend does not maintain or explicitly evict a second image cache when a node is destroyed.

`img` load completion and failure are reported as native `load` and `error` events. Each subscribed `img` with a retained `src` renders inside a small observer element that checks the GPUI asset state through the non-notifying asset query; the inner `Img` element's own notifying query drives the re-render that makes the terminal state visible. The observer keeps `(src, delivered)` in GPUI element state, so delivery is per node per source with no global scan and no manual cleanup: dropping the source (or its last subscription) drops the observer and its state, and a new source resets delivery. The first terminal state per source emits once: a decoded image emits `load`, a network, status, decode, or asset error emits `error`. Images without a source, or without a `load`/`error` subscription in path, are never wrapped and emit nothing.
...
The native protocol event vocabulary contains `click`, `dblclick`, `mousedown`, `mouseup`, `mouseenter`, `mouseleave`, `mousemove`, `keydown`, `keyup`, `input`, `change`, `focus`, `blur`, `scroll`, `load`, `error`, and the non-DOM extension `mousedownoutside`. JSX props keep React-style casing such as `onMouseDown`; protocol event names use their explicitly defined lowercase forms.

Application-defined custom events remain entirely in JavaScript and do not enter the native protocol vocabulary. Native event types carry propagation metadata. `mouseenter`, `mouseleave`, `focus`, `blur`, element `scroll`, and image `load`/`error` are non-bubbling; ordinary pointer/button/key events such as `click`, `mousedown`, `mouseup`, `mousemove`, `keydown`, and `keyup` bubble. Capture and target behavior follows each event type's metadata.
...
`input` and `textarea` are backed by `gpui-base`'s native input engine, which implements GPUI's `EntityInputHandler`/`ElementInputHandler` pattern for focus, IME, selection, mouse selection, marked ranges, platform text-input integration, and bounded/coalesced undo history. Retend does not duplicate that editor state; its adapter owns only retained node association, controlled-value reconciliation, imperative bridge commands, and event translation. The adapter projects the palette's selection color onto each editor, because an unset selection resolves to the palette's accent, which is a near-white surface color and therefore invisible against a control's own surface. `input` is single-line; `textarea` adds multi-line wrapping and `minRows`/`maxRows` auto-sizing.
...
Static and transitioned `hover`, `focused`, and `active` styles are native-owned. JavaScript publishes reactive author-style snapshots for the base style and for each pseudo-state. Rust owns current hover/pressed state and the focused node, resolves precedence as `active > focused > hover > base`, and applies immediate refinements or transition retargeting through the native motion state machine.
...
Retargeting, cancellation/snap behavior, and pseudo-state precedence live in the same native engine. The animatable set is `width`, `height`, `top`, `right`, `bottom`, `left`, `opacity`, `borderRadius`, `backgroundColor`, `color`, and `borderColor`. Additional properties can be added through the native transition engine. `MOTION.md` defines the public transition semantics, including longhands, soft-fail behavior, retargeting from the visible value, initial-render behavior, and commit/coalescing boundaries.
