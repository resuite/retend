...
Each macOS build also emits a compressed `.dmg` containing the `.app`. Bundles are signed ad-hoc rather than with a Developer ID, so they are not notarized. Developer ID signing and notarization require a paid Apple Developer Program membership, and that cost is deliberately not imposed on local builds or first public releases. A distribution that wants the warning-free path adds Developer ID signing and notarization without changing the bundle layout.
