...
Each macOS build also emits a compressed `.dmg` containing the `.app` and an `INSTALL.txt` next to it. Bundles are signed ad-hoc rather than with a Developer ID, so they are not notarized. Developer ID signing and notarization require a paid Apple Developer Program membership, and the cost is deliberately not imposed on local or first public builds: the install note explains the Gatekeeper prompt and the one-time quarantine removal that stand in for notarization. A distribution that wants the warning-free path can add Developer ID signing and notarization later without changing the bundle layout.

Because that consequence is invisible until a stranger downloads the app, packaging emits a build warning whenever the bundle is signed ad-hoc. It names the exact failure a user will hit and what to configure. This is deliberately a warning rather than an error: local builds and bundles distributed through channels that do not set the quarantine flag (a `curl` installer, Homebrew, building from source) are legitimate without a Developer ID, and the warning should not block them.

Signing and notarization are configured through `retendGpui({ signing })`. `signing.identity` (or `RETEND_GPUI_SIGN_IDENTITY`) is a Developer ID identity. When present, packaging signs in dependency order — the bundled native addon, then the SEA executable, then the bundle — with `--options runtime`, `--timestamp`, and a JIT-capable entitlements plist (`signing.entitlements` overrides the generated default, which grants `allow-jit`, `allow-unsigned-executable-memory`, and `disable-library-validation`, all required for V8 under the hardened runtime). Notarization runs when credentials are resolvable: `signing.notaryProfile`, else `RETEND_GPUI_NOTARY_PROFILE`, else an App Store Connect API key (`APPLE_API_KEY`/`APPLE_API_KEY_ID`/`APPLE_API_ISSUER`), else `APPLE_ID`/`APPLE_TEAM_ID`/`APPLE_APP_SPECIFIC_PASSWORD`. The disk image is submitted with `notarytool submit --wait`, the ticket is stapled, and Gatekeeper acceptance is verified. A signed-but-unnotarized build warns too, because Gatekeeper still blocks it on download. None of this runs, and no Apple server is contacted, unless an identity and credentials are supplied.
...
Ordinary production assets such as images, fonts, JSON/data files, and shaders are emitted as real files in the native application bundle's resource directory rather than embedded into the SEA executable. The `gpui` environment build enables `emitAssets`, pins `assetsDir` to `assets`, and sets `assetsInlineLimit` to `0`, so imported assets become files under `assets/` and imports resolve to root-relative URLs (`/assets/<name>`). The runtime resolves those against the resource directory and hands the native renderer a `file:` URL; in development they resolve against the Vite root instead. Relative paths without a leading slash are outside the native image surface. Native `.node` bindings likewise remain filesystem resources required by the executable at runtime.
