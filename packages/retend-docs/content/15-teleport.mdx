...
function ThemedTooltip() {
  // We can still read the ThemeScope from our parent component,
  // even though we are teleporting our HTML to the very bottom of the page!
  const theme = useScopeContext(ThemeScope);

  return (
    <Teleport to="body">
      <div class={`tooltip tooltip-${theme.get()}`}>Helpful information!</div>
    </Teleport>
  );
}
```
