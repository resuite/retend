...
If you need data to load during SSR, serialize into the hydration payload, and resume on the client, use `serverResource` from `retend-server`.
...
function UserPage(props) {
  const user = Cell.derivedAsync(async () =>
    serverResource(['user', props.userId], () =>
      fetch(`/api/users/${props.userId}`).then((response) => response.json())
    )
  );

  return If(user, {
    true: (value) => <h1>{value.name}</h1>,
  });
}
```

The first argument defines the cache key. If that key depends on reactive values, put `serverResource` inside `Cell.derivedAsync()` and read those values there so the async cell can refetch when the key changes.
