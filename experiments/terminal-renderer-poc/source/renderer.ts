...
class BoxNode extends BaseNode implements TerminalContainerNode {
    ...
    measure(availableWidth?: number) {
        // --- SIZING PASS: Calculate sizes recursively ---
         ...
         const padding = getValue(style.padding) || 0;
         ...
         if (this.width) childAvailableWidth = this.width - padding * 2 - borderWidth;
         else if (isRoot && style.width) childAvailableWidth = getValue(style.width) - padding * 2 - borderWidth;
         else if (availableWidth) childAvailableWidth = availableWidth - padding * 2 - borderWidth;
         ...
         while (i < this.children.length) {
             ...
             if (child.type === "text-inner") {
                 ...
                 if (group.length > 1) {
                     ...
                     first.measure(childAvailableWidth);
         ...
         if (!this.width && !isRoot) {
             this.width = (flexDirection === "column" ? maxChildWidth : totalChildWidth) + padding * 2;
         }
         if (!this.height && !isRoot) {
             this.height = (flexDirection === "column" ? totalChildHeight : maxChildHeight) + padding * 2;
         }
    ...
    layout() {
		...
		const flowChildren = this.children.filter((c) => getValue(c.props.style?.position) !== "absolute");
        ...
        for (const child of this.children) {
			const childStyle = child.props.style || {};
			if (getValue(childStyle.position) === "absolute") {
				const left = childStyle.left !== undefined ? getValue(childStyle.left) : 0;
            ...
            if (flexDirection === "column") {
                if (alignItems === "center") childX = this.x + padding + Math.floor((availWidth / 2 - child.width / 2));
                else if (alignItems === "flex-end") childX = this.x + this.width - padding - child.width;
                else childX = this.x + padding;
                childY = currentY;
                currentY += child.height + (childIndex > 0 ? 0 : 0);
            } else {
                if (alignItems === "center") childY = this.y + padding + Math.floor((availHeight / 2 - child.height / 2));
                else if (alignItems === "flex-end") childY = this.y + this.height - padding - child.height;
...
class TextNode extends BaseNode implements TerminalTextNode {
    text: string;
...
export class TerminalRenderer {
    ...
    render() {
        ...
        this.root.measure(this.host.width);
	...
	isActive(_node: TerminalNode): boolean { return true; }
	onViewChange(_processor: () => void) {}
    createGroup(input?: TerminalNode | TerminalNode[]) { return Array.isArray(input) ? input : [input || new MarkerNode('')]; }
    unwrapGroup(group: TerminalNode[]) { return group; }
    createGroupHandle(group: TerminalNode[]): TerminalHandle { 
        const s = new MarkerNode("s"); const e = new MarkerNode("e"); 
        group.unshift(s); group.push(e); return [s, e]; 
    }
    ...
    write(handle: TerminalHandle, newContent: TerminalNode[]) {
        ...
        parent.children.splice(startIndex + 1, endIndex - startIndex - 1, ...newContent);
        newContent.forEach(n => n.parent = parent);
        this.requestRender();
    }
    ...
    handleComponent(tag: any, props: any): TerminalNode | TerminalNode[] {
        const c = tag(props);
        return createNodesFromTemplate(c, this) as TerminalNode[];
    }
    
    // Unused strict methods
    finalize(n: TerminalNode) { return n; }
    isGroup(n: unknown): n is TerminalNode[] { return Array.isArray(n); }
    isNode(n: unknown): n is TerminalNode { return n instanceof BaseNode; }
    handlePromise() { return new MarkerNode('p'); }
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    reconcile(handle: any, options: any) {
        // Minimal impl
        this.write(handle, options.nodes || []);
    }
    selectMatchingNodes() { return []; } 
	selectMatchingNode() { return null; }
	saveContainerState() { return null; }
	restoreContainerState() {}
    updateText(t: string, n: TerminalTextNode) { 
        n.text = t; 
        // Destroy old widget so it gets recreated
        if ((n as TextNode).blessedWidget) {
            (n as TextNode).blessedWidget?.destroy();
            (n as TextNode).blessedWidget = null;
        }
        this.requestRender(); 
        return n; 
    }
