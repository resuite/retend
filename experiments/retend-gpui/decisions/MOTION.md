...
When multiple pseudo-states define the same transitioned property, Retend GPUI uses the explicit precedence `active > focused > hover > base`. Pressing while hovered transitions from the hover value to the active value. Releasing while still inside transitions back to the hover value; releasing outside transitions back to the base value.
